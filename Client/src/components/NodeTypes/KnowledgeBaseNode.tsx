...
export const KnowledgeBaseNode: React.FC<NodeProps<NodeData>> = ({ id, selected, data }) => {
  const updateNodeConfig = useWorkflowStore((state) => state.updateNodeConfig);
  
  const [embedding, setEmbedding] = useState(data.config?.embeddingModel || 'text-embedding-3-large');
  ...
  const handleFileChange = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      updateNodeConfig(id, { uploadedFile: file, uploadedFileName: file.name, name: 'KnowledgeBase' });
    }
  }, [id, updateNodeConfig]);

  const handleDeleteFile = useCallback(() => {
    setUploadedFileName('');
    updateNodeConfig(id, { uploadedFile: null, uploadedFileName: '', name: 'KnowledgeBase' });
  }, [id, updateNodeConfig]);
  ...
  return (
    <div className={`bg-white rounded-lg border-2 p-4 min-w-[280px] ${selected ? 'border-blue-500' : 'border-gray-200'}`}>
      ...
      <div className="space-y-3">
        <div>
          <label className="text-xs text-gray-600">Let LLM search info in your file</label>
        </div>
        <div>
